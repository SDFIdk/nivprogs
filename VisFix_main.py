...
class MainFrame(wx.Frame):
    def __init__(self,parent,title,Ini):
        ...
        self.Bind(wx.EVT_MENU,self.OnUpdateIDs,UpdateIDs)
    ...
    def OnAddIDs(self,event):
        #Add ids that do not exists
        self.AddOrUpdateIDs(False)
    def OnUpdateIDs(self,event):
        #update or add ids (upsert)
    ...
    def AddOrUpdateIDs(self,update=False):
        ...
        if file!=-1:
            ...
            if len(Stations)>0:
                ...
                if dlg.WasOK():
                    if update:
                        nd=self.Map.data.UpdateOrCreatePoints(Stations)
                    else:
                        nd=self.Map.data.CreateNewPoints(Stations)
                dlg.Destroy()
                n=self.Map.data.GetNChanged()
                self.Log("Datafil opdateret. Opdateringer ialt: %i" %n)
            else:
