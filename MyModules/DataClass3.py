...
class PointData(object):
    ...
    def CreateNewPoints(self,stations,tabt=False):
        ...
        self.con.commit()
        return nd
    def UpdateOrCreatePoints(self,stations, tabt=False):
        ...
        for station in stations.keys():
            ...
            if res is None:
                ...
                nd+=1
                id=res[0]
                #do not change tabtgaaet in this case
                self.cur.execute("update main set hsnavn=?,gmginavn=?,gpsnavn=? where id=?",(station,gmginavn,gpsnavn,id))
        self.con.commit() #we shouldn't neeed a commit after each insert....
        return nd
